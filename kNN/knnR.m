all_train = [trainJazzR trainRockR trainClassicalR];
all_labels = [labelJazz labelRock labelClassical];
all_test = [testJazzR testRockR testClassicalR];
...
for kk=1:length(k)
    ...
    rateR(kk) = (correct/length(predicted_label))*100;
end
[a,b]=max(rateR);
disp('----------results using Spectral Rolloff----------------')
