all_train = [trainJazzS trainRockS trainClassicalS];
all_labels = [labelJazz labelRock labelClassical];
all_test = [testJazzS testRockS testClassicalS];
...
for kk=1:length(k)
    ...
    rateS(kk) = (correct/length(predicted_label))*100;
end
[a,b]=max(rateS);
disp('----------results using Spectral Spread----------------')
