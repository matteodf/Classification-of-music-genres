all_train = [trainJazzZ trainRockZ trainClassicalZ];
all_labels = [labelJazz labelRock labelClassical];
all_test = [testJazzZ testRockZ testClassicalZ];
...
for kk=1:length(k)
    ...
    rateZ(kk) = (correct/length(predicted_label))*100;
end
[a,b]=max(rateZ);
disp('----------results using ZCR----------------')
